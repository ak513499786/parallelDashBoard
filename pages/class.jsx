...
export default function Dashboard() {
  ...
  const groupedSchedules = schedules.reduce((acc, schedule) => {
    const day = new Date(schedule.date).toLocaleDateString('en-US', { weekday: 'long', day: 'numeric' });
    if (!acc[day]) acc[day] = [];
  ...
  return (
    <>
      ...
      {Object.keys(groupedSchedules).map((day, index) => (
        <div key={index} className="py-[18.5px] max-[500px]:flex-col max-[500px]:gap-[20px] border-b-[1px] border-[#C4C4C4] pl-[18.3px] pr-[15.6px] flex justify-between max-md:px-[0px] max-sm:px-[5px]">
          <h1 className="text-base opacity-70">{day}</h1>
          <div className="w-[478.35px] max-[1350px]:w-auto max-[1040px]:w-[210px]">
            {groupedSchedules[day].map((schedule, i) => (
              <div key={i}>
                <h1 className="text-base opacity-70 mb-[4px]">
                  Class: {schedule.startTime} to {schedule.endTime}
                </h1>
                <p className="text-[20px]">{schedule.title}</p>
              </div>
            ))}
          </div>
        </div>
    ...
    <section className="mt-[24.62px] h-[659px] rounded-[6px] bg-white">
      ...
      <div className="pt-[33px] max-md:px-[20px] max-sm:px-[15px] pl-[25px] pr-[41.64px]">
        {modules.map((module, index) => (
          <div key={index} className="py-[10px] max-smallerphone:gap-[6px] border-b-[1px] flex gap-[12px]">
            <Image
              src="/Frame 18.svg"
